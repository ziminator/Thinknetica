...
class Interface
  ...
  TEXT_CONST = {
    ...
    :volume_busy => "Введите значение объёма, который необходимо занять в вагоне"
  }
  ...
  def puts_add_pass_wagon(train, seats)
    puts "К поезду № - #{train.number} добавлен пассажирский вагон, свободных мест - #{seats}. Всего добавлено вагонов #{train.wagons.count}."
  end

  def puts_add_cargo_wagon(train, volume)
    puts "К поезду № - #{train.number} добавлен товарный вагон общим объёмом - #{volume}. Всего добавлено вагонов #{train.wagons.count}"
  end

  def puts_pass_wagons(train)
    train.wagons.each.with_index(1) { |train, index|  puts "В вагоне № - #{index}: всего мест-#{train.total}, свободных мест-#{train.free}, занято мест-#{train.busy}" }
  end

  def puts_cargo_wagons(train)
    train.wagons.each.with_index(1) { |train, index|  puts "Вагон № - #{index}: общий объём-#{train.total}, свободный объём вагона-#{train.free}, занятый объём вагона-#{train.busy}" }
  end
  ...
  def puts_each_train(train)
    puts train.type == :passenger ? "Поезд №: #{train.number}, тип: \"Пассажирский\", вагонов #{train.wagons.count}" : "Поезд №: #{train.number}, тип: \"Товарный\", вагонов #{train.wagons.count}"
  end
