...
class Main
  ...
  def run
    ...
    loop do
      ...
      when 13 then add_seat_volume             #Занять место в пассажирском вагоне
      when 14 then show_wagons                 #Занять объём грузового вагона
      when 15 then show_trains_on_stations     #Отобразить все поезда на всех станциях
      when 20 then test_station_objects        #Список объектов станций (тест)
      when 21 then test_find_objects           #Класс find (тест)
      end
    end
  end
